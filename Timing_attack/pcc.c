...
pcc_consolidate (pcc_context *ctx)
{
  ...
  n = (double) (ctx->nr);
  tmp1 = sqrt (n * ctx->x2 - ctx->x * ctx->x);
  if (tmp1 == 0.0)
  {
  ...
  for (i = 0; i < ctx->ny; i++)
    {
      tmp2 = sqrt (n * ctx->y2[i] - ctx->y[i] * ctx->y[i]);
      if (tmp2 == 0.0)
      {
        ERROR (, -1, "Y%d variance equals zero; could it be that your Y%d variable is constant?", i, i);
      }
      ctx->pcc[i] = (n * ctx->xy[i] - ctx->x * ctx->y[i]) / (tmp1 * tmp2);
    }
}
